...
func int C_AmIWeaker (var C_Npc slf, var C_Npc oth)
{
	...
	if ((3*slf.level) <= oth.level) //&& ((slf =! VLK_5089_Tragaz) || (slf =! VLK_5090_Tragaz) || (slf =! VLK_5088_Tragaz)) //fix rocky quest
	{
		PrintDebugNpc	(PD_ZS_DETAIL, "...yes" );
		return 			TRUE;
	}
...
func int C_NpcIsDangerousMonster(var C_NPC slf, var C_NPC oth)
{
	...
	if	(C_NpcIsOrc(oth))//edit by pawbuj, inaczej npc nie atakuja orków
...
func int C_NpcIsOrc(var C_NPC slf)
{
	...
	if (slf.guild > GIL_SEPERATOR_ORC)	
	//||( slf.guild == GIL_ORCSHAMAN )//fix bug pawbuj 
	{
		PrintDebugNpc	(PD_ZS_DETAIL,"...true");
		return TRUE;
	}
...
func int C_NpcIsDangerousMonster(var C_NPC slf, var C_NPC oth)
{
	...
	if	(!C_NpcIsOrc(slf)&& C_NpcIsOrc(oth))//edit by pawbuj, inaczej npc nie atakuja orków
	||	((C_NpcIsMonster(oth))  
	&& 	(Wld_GetGuildAttitude(oth.guild,slf.guild) == ATT_HOSTILE)
	&&	(!oth.aivar[AIV_MM_PARTYMEMBER])
	&&	(!C_NpcIsDown(oth)))
