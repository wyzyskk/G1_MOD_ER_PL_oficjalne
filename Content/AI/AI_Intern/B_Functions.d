...
//	Log_GetTopicStatus
//	=================
//	Ikarus func! Gothic ER
//////////////////////////////////////////////////////////////////////////
const int Log_GetLogManager_Ptr = 10328180; 
...
func void B_LogEntry(var string topic, var string entry)
{
	...
	if (mis == LOG_SUCCESS) {
	var string InfA;
	const string inf1 = "Wykonano zadanie: ";
	infA = ConcatStrings (inf1,topic);
	PrintScreen (infA, -1,-1,"font_new_10_green.tga",2);	
	};
	if (mis == LOG_RUNNING) {
    const string inf2 = "Rozpoczêto zadanie: ";
	var string InfA2;
	InfA2 = ConcatStrings (inf2,topic);
	PrintScreen (InfA2, -1,53,"font_old_10_white.tga",2);	
	};
	
	if (mis == LOG_FAILED) {
     const string inf3 = "Popsuto zadanie: ";
	var string InfA3;
	InfA3 = ConcatStrings (inf3,topic);
	PrintScreen (InfA3, -1,55,"font_new_10_red.tga",2);	
	};*/
};
...
func void B_LogStatus (var string topic, var int type)
{
	...
	//B_Y_LOGENTRY();
	var string Anz_LogS;
	Log_SetTopicStatus (topic,type);

	if (type == Log_Success)
	{
	Anz_LogS = ConcatStrings ("Zadanie '",topic);
	Anz_LogS = ConcatStrings (Anz_LogS,"'");
	Anz_LogS = ConcatStrings (Anz_LogS," ukoñczone pomyœlnie");
	PrintScreen		(Anz_LogS, -1,_YPOS_MESSAGE_LOGENTRY,"font_old_20_white.tga",((_TIME_MESSAGE_LOGENTRY*3)/2));
	}
	else if (type == Log_Failed)
	{
	Anz_LogS = ConcatStrings ("Zadanie '",topic);
	Anz_LogS = ConcatStrings (Anz_LogS,"'");
	Anz_LogS = ConcatStrings (Anz_LogS," jest zepsute");
	PrintScreen		(Anz_LogS, -1,_YPOS_MESSAGE_LOGENTRY,"font_old_20_white.tga",((_TIME_MESSAGE_LOGENTRY*3)/2));
	}
	else if (type == Log_Running)
	{
	Anz_LogS = ConcatStrings ("Nowe zadanie: ",topic);
	PrintScreen		(Anz_LogS, -1,_YPOS_MESSAGE_LOGENTRY,"font_old_20_white.tga",((_TIME_MESSAGE_LOGENTRY*3)/2));
	};
};

func void B_NewAchivement(var string osiagniecie, var string nazwa)
{
	PrintDebugNpc	(PD_ZS_DETAIL,	"OSI¥GNIÊCIE");

	Log_AddEntry	(osiagniecie, nazwa);
	
	var string printLog;
	printLog = ConcatStrings ("Nowe osi¹gniêcie: ",nazwa);
	printLog = ConcatStrings (printLog, ".");
	PrintScreen		(printLog, -1,60,"font_old_10_white.tga",_TIME_MESSAGE_LOGENTRY);
	Snd_Play 		("LogEntry");
};
...
func void B_GiveInvItems(var C_NPC giver, var C_NPC taker, var int itemInstance, var int amount)
{
	...
	if	Npc_IsPlayer(giver)
	{
		if (itemInstance == ItMiNugget)
		{
			...
			PrintS_Ext(msg, COL_Red);
			//MultiText (msg, "FONT_OLD_10_WHITE.TGA", -1, -1, COL_White, -2500, 0, 2000, "DLG_AMBIENT.TGA", 0, 0, A8_SlowEnd, 5000);
		}
		else
		{
		    if amount == 1
	...
	else if Npc_IsPlayer(taker)
	{
		if (itemInstance == ItMiNugget)
		{
			...
			PrintS_Ext(msg, COL_Green);
			//MultiText (msg, "FONT_OLD_10_WHITE.TGA", -1, -1, COL_White, 3000, 0, 2000, "DLG_AMBIENT.TGA", 0, 0, A8_SlowEnd, 5000);
		}
		else
		{
		    if amount == 1
		    {
			    ...
			    //PrintScreen	(msg, -1,_YPOS_MESSAGE_TAKEN,"FONT_OLD_10_WHITE.TGA",_TIME_MESSAGE_TAKEN);\
				PrintS_Ext(msg, COL_Green);
				//MultiText (msg, "FONT_OLD_10_WHITE.TGA", -1, 3000, COL_White, 3000, 0, 2000, "DLG_AMBIENT.TGA", 0, 0, A8_SlowEnd, 5000);
			}   
			else
		    {
			    ...
			    //PrintScreen	(msg, -1,_YPOS_MESSAGE_TAKEN,"FONT_OLD_10_WHITE.TGA",_TIME_MESSAGE_TAKEN);
				PrintS_Ext(msg, COL_Green);
				//MultiText (msg, "FONT_OLD_10_WHITE.TGA", -1, 2000, COL_White, 3000, 0, 2000, "DLG_AMBIENT.TGA", 0, 0, A8_SlowEnd, 5000);
			}; 
		};
	};	
};
