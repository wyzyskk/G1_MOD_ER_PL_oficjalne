...
const int PoisonDamagePerSec = 3;
const int PosionDuration = 30;
func void FixPoison() {
	if (!FF_Active(DoPoisonHero))
	&& (hero.aivar[AIV_POISON]) {
		var oCViewStatusBar bar; bar = _^(MEM_Game.hpBar);
		_View_SetTexture(bar.value_bar, "BAR_HEALTH.tga");
		bar.texValue = "BAR_HEALTH.tga";
		hero.aivar[AIV_POISON] = false;
		FF_Remove(FixPoison);
	};
};
func void CheckPoison(var C_Npc att) {
print ("check");
	if ((Hlp_GetInstanceID(att)==Hlp_GetInstanceID(WaranJadowity))
	|| (Hlp_GetInstanceID(att)==Hlp_GetInstanceID(Bloodfly))
	|| (Hlp_GetInstanceID(att)==Hlp_GetInstanceID(AngryBloodfly))
	|| (Hlp_GetInstanceID(att)==Hlp_GetInstanceID(HelpBrothers_Bloodfly))) {
		...
		//if (r_minMax(0, 100) >= 5) {
			...
			&& (!Npc_IsDead(hero)) {
				...
				var oCViewStatusBar bar; bar = _^(MEM_Game.hpBar);
				_View_SetTexture(bar.value_bar, "BAR_POISON.tga");
				bar.texValue = "BAR_POISON.tga";

				FF_ApplyOnceExt(DoPoisonHero, 1000, PosionDuration); // 1000(czas w ms) * 30(iloœæ wywo³añ) = 30 sekund 
				FF_ApplyOnce(FixPoison);
				hero.aivar[AIV_POISON] = true;
			};
		//};
	};
};

func void DoPoisonHero() {
	var C_Npc her; her = Hlp_GetNpc(hero);
	her.attribute[ATR_HITPOINTS] -= PoisonDamagePerSec;
	if (Npc_IsDead(her)) {
		FF_Remove(DoPoisonHero);
		her.aivar[AIV_POISON] = false;
	};
};
