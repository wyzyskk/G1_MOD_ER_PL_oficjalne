...
// Spells, die abgebrochen werden, wenn beim loslassen der Taste noch nicht gecastet 
	...
	if (Npc_GetActiveSpell(self) == SPL_TELEPORT_ER		)	{	return	SPL_SENDSTOP;	};//er
	if (Npc_GetActiveSpell(self) == SPL_TELEPORT_OH		)	{	return	SPL_SENDSTOP;	};//er
	if (Npc_GetActiveSpell(self) == SPL_TELEPORT_BC		)	{	return	SPL_SENDSTOP;	};//er
	if (Npc_GetActiveSpell(self) == SPL_SUMMONORCSKE	)	{	return	SPL_SENDSTOP;	};//er
