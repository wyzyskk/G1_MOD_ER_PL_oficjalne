...
func void LeGo_InitGamestart(var int f) {
    ...
    if(f & LeGo_Cursor) {
        HookEngine(5013392, 5, "_CURSOR_GETVAL");
    };
    if(f & LeGo_Random) {
        r_DefaultInit();
    };
    ...
    if(f & LeGo_FrameFunctions) {
        HookEngine(oCGame__Render, 7, "_FF_HOOK");
    };
    ...
    if(f & LeGo_Sprite) {
        HookEngine(zRND_D3D__EndFrame, 6, "_Sprite_DoRender");
    };
...
func void LeGo_Init(var int flags) {
    ...
    MEM_InitAll();
	 TurnOffFrameLimit(); // Do it on your own, I recommend to call this function :)
