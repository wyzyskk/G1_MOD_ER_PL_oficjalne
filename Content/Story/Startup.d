...
func VOID STARTUP_SUB_NEWCAMP ()
{
	...
	//er 1.4
	Wld_InsertNpc				(ORG_952_Organisator,"NC_DAM");
	...
	Wld_InsertNpc				(BAU_924_Bauer,"NC_DAM"); //CF1 bei Rufus
...
FUNC VOID STARTUP_SUB_OLDCAMP ()
{
	...
	//informacja: npc nie jest ju¿ spawnowany w Old_camp, pojawia siê tylko w startup Abadoned_mine
	//Wld_InsertNpc				(NON_2094_Glest,"OC1");
	...
	Wld_InsertNpc				(VLK_3999_Tippler,"OC1"); //swiadek zabójstwa Neka
	...
	Wld_InsertNpc				(GRD_7007_Mirzo,"OC1"); //straznik skrzyñ z rud¹
	Wld_InsertNpc				(GRD_7008_Tukash,"OC1"); //handlarz, ucieka na 4Ch do bandytów
	...
	Wld_InsertNpc				(GRD_2017_Artch,"OC1"); //glówne wejscie
...
//	Wld_InsertNpc				(Grd_210_Scatty,"OC1"); lol? 2 razy?
	...
	/*
	Straznicy magazynu za starym obozem
	
	Wld_InsertNpc		(GRD_8020_Straznik,"OC1");
	Wld_InsertNpc		(GRD_8021_Straznik,"OC1");
	Wld_InsertNpc		(GRD_8022_Straznik,"OC1");
	*/
};
...
FUNC VOID INIT_SUB_OLDCAMP ()
{
	...
	**************************************************************/
	...
	//Wld_AssignRoomToGuild("hütte2",GIL_VLK); ?? Lorenzo
...
//	Wld_InsertNpc		(Bloodfly,"SPAWN_OW_BLOODFLY_12"); **fix 1.24 mysliwi **
	...
	// in den Höhlen //er.1.4 edited gothic1210
	...
	Wld_InsertNpc		(BlackGobboMace,"GOBBO_MASTERCAVE7");
	Wld_InsertNpc		(Gol_Umik,"GOBBO_MASTERCAVE8"); //boss er 1.4.5
	Wld_InsertNpc		(Gobbo_Warrior,"GOBBO_MASTERCAVE8");
...
//	Wld_InsertNpc				(Waran,"OW_PATH_BANDITOS01");
 ...
 Wld_InsertNpc		(Troll,"OW_RUIN_TROLL");	//wird erst später in B_Story_LeftFM() gespawnt!
	...
	// Ork-Stadt: Podziemia (Miasto Orków)
...
// Kleine Klasse die benötigt wird
class zCSkyState {
    var int time;                //zREAL
...
// Gibt an welche Farbe zu welcher Uhrzeit angezeigt wird.
// Lücken werden sauber interpoliert
// WICHTIG: Nacht muss < 24 beginnen und > 0 enden!
...
// Kolorki se ustaw xD
/*
const int PolyColors[12] = 
{//  r,   g,   b
    167, 141, 159, // swit
    82,  109, 198, // ranek 220, 220, 220, // dzien
    139, 46,  0,   // wieczór
    18,  16,  60   // noc
...
func void Colors(var int r_fog, var int g_fog, var int b_fog, var int r_poly, var int g_poly, var int b_poly) 
{
    var zCSkyState s;

    if (MEM_SkyController.state0 != 0)
    {
    s = MEM_PtrToInst(MEM_SkyController.state0);
    s.fogColor[0] = r_fog;
    s.fogColor[1] = g_fog;
    s.fogColor[2] = b_fog;
    s.polyColor[0] = r_poly;
    s.polyColor[1] = g_poly;
    s.polyColor[2] = b_poly;
    };

    if (MEM_SkyController.state1 != 0)
    {
    s = MEM_PtrToInst(MEM_SkyController.state1);
    s.fogColor[0] = r_fog;
    s.fogColor[1] = g_fog;
    s.fogColor[2] = b_fog;
    s.polyColor[0] = r_poly;
...
var int i; var int p;
    ...
    if(!SkyColorsFlt) {
        ...
        if(i < 12) {
            ...
            MEM_WriteStatArr(FogColors, i, mkf(MEM_ReadStatArr(FogColors, i)));
                  MEM_WriteStatArr(PolyColors, i, mkf(MEM_ReadStatArr(PolyColors, i)));
            i += 1;
    ...
    var int r_fog; var int g_fog; var int b_fog;
    var int r_poly; var int g_poly; var int b_poly;

    // Ausnahme bei Nacht, da Sprung
    if(lef(hour, SkyTimes[7])||gef(hour, SkyTimes[6])) {
        r_fog = FogColors[9];
        g_fog = FogColors[10];
        b_fog = FogColors[11];
        r_poly = PolyColors[9];
        g_poly = PolyColors[10];
        b_poly = PolyColors[11];
    }
    else {
        i = 0;
        p = MEM_StackPos.position;
        if(i < 8) {
            if(lef(hour, MEM_ReadStatArr(SkyTimes, i))) {
                if(i == 1||i == 3||i == 5) {
                    i = ((i-1)/2)*3;
                    r_fog = MEM_ReadStatArr(FogColors, i);
                    g_fog = MEM_ReadStatArr(FogColors, i+1);
                    b_fog = MEM_ReadStatArr(FogColors, i+2);
                      r_poly = MEM_ReadStatArr(PolyColors, i);
                    g_poly = MEM_ReadStatArr(PolyColors, i+1);
                    b_poly = MEM_ReadStatArr(PolyColors, i+2);
                    i = 8;
                    MEM_StackPos.position = p;
                };
                ...
                var int j; j = i-3;
                if(j < 0) { j = 9; };
                r_fog = addf(MEM_ReadStatArr(FogColors, j), mulf(subf(MEM_ReadStatArr(FogColors, i), MEM_ReadStatArr(FogColors, j)), pm));
                g_fog = addf(MEM_ReadStatArr(FogColors, j+1), mulf(subf(MEM_ReadStatArr(FogColors, i+1), MEM_ReadStatArr(FogColors, j+1)), pm));
                b_fog = addf(MEM_ReadStatArr(FogColors, j+2), mulf(subf(MEM_ReadStatArr(FogColors, i+2), MEM_ReadStatArr(FogColors, j+2)), pm));
                        r_poly = addf(MEM_ReadStatArr(PolyColors, j), mulf(subf(MEM_ReadStatArr(PolyColors, i), MEM_ReadStatArr(PolyColors, j)), pm));
                g_poly = addf(MEM_ReadStatArr(PolyColors, j+1), mulf(subf(MEM_ReadStatArr(PolyColors, i+1), MEM_ReadStatArr(PolyColors, j+1)), pm));
                b_poly = addf(MEM_ReadStatArr(PolyColors, j+2), mulf(subf(MEM_ReadStatArr(PolyColors, i+2), MEM_ReadStatArr(PolyColors, j+2)), pm));
                i = 8;
                MEM_StackPos.position = p;
            };
            i += 1;
    ...
    Colors(r_fog, g_fog, b_fog, r_poly, g_poly, b_poly);
};
...
FUNC VOID INIT_WORLD ()
{
	...
	//ScreenFx_Init();
	//ScreenFX2_Init();
	//FF_ApplyOnce(ScreenFx_func);
	//FF_ApplyOnce(ImmersiveHUD_func);
...
Wld_SendTrigger("WYZWALACZ");
	...
	//-------- 3. Höhle --------
	Wld_InsertNpc		(TPL_5083_OldAlchemyMan,"GRYD_025");
	...
	//-------- Große Halle --------
	//Wld_InsertNpc		(Orcskeleton_Varr,		"GRYD_061");
	//Wld_InsertNpc		(Orcskeleton_Varr,		"GRYD_062");
	//Wld_InsertNpc		(Orcskeleton_Varr,		"GRYD_058");
	//Wld_InsertNpc		(Orcskeleton_Varr,		"GRYD_068");
	...
	//Wld_InsertNpc		(Orcskeleton_Varr,	"GRYD_ORCSKE4");
	
	/*Wld_InsertNpc		(OrcWarrior2,		"GYD_500");
	Wld_InsertNpc		(OrcScoutGYD,		"GYD_501");
	Wld_InsertNpc		(OrcScoutGYD,		"GYD_502");
	Wld_InsertNpc		(OrcScoutGYD,		"GYD_503");
	Wld_InsertNpc		(OrcScoutGYD,		"GYD_504");
	Wld_InsertNpc		(OrcScoutGYD,		"GYD_505");
	Wld_InsertNpc		(OrcScoutGYD,		"GYD_506");
	Wld_InsertNpc		(OrcScoutGYD,		"GYD_507");
	Wld_InsertNpc		(OrcScoutGYD,		"GYD_508");
	Wld_InsertNpc		(OrcScoutGYD,		"GYD_509");*/
};
...
Wld_SendTrigger("WYZWALACZ");
	...
	//NEW
	Wld_InsertNpc				(NON_5091_MrocznyZeno,"TPL_ZENEK" ); 
	Wld_InsertNpc				(KDF_8084_CorristoOT,"TPL_408" ); 
	// Poszukiwacze
	Wld_InsertNpc				(NOV_5060_Poszukiwacz,"TPL_408" ); 
	Wld_InsertNpc				(NOV_5061_Poszukiwacz,"TPL_408" ); 
	Wld_InsertNpc				(NOV_5062_Poszukiwacz,"TPL_408" ); 
	Wld_InsertNpc				(NOV_5063_Poszukiwacz,"TPL_408" ); 
	Wld_InsertNpc				(NOV_5064_Poszukiwacz,"TPL_408" ); 
	Wld_InsertNpc				(NOV_5065_Poszukiwacz,"TPL_408" ); 
	Wld_InsertNpc				(NOV_5066_Poszukiwacz,"TPL_408" );
	Npc_SetPermAttitude (NOV_5060_Poszukiwacz, ATT_HOSTILE);
	Npc_SetPermAttitude (NOV_5061_Poszukiwacz, ATT_HOSTILE);
	Npc_SetPermAttitude (NOV_5062_Poszukiwacz, ATT_HOSTILE);
	Npc_SetPermAttitude (NOV_5063_Poszukiwacz, ATT_HOSTILE);
	Npc_SetPermAttitude (NOV_5064_Poszukiwacz, ATT_HOSTILE);
	Npc_SetPermAttitude (NOV_5065_Poszukiwacz, ATT_HOSTILE);
	Npc_SetPermAttitude (NOV_5066_Poszukiwacz, ATT_HOSTILE);
	//--------------------- Razors -------------------------------------
	Wld_InsertNpc				(SnapperDragon, "BEAST1"); 
	Wld_InsertNpc				(SnapperDragon, "BEAST2"); 
	Wld_InsertNpc				(SnapperDragon, "BEAST3"); 
	Wld_InsertNpc				(SnapperDragon, "DIEGO_TPL2"); 
	// ---------------------------------------------------------------
	// ------------------ PSI ----------------------------------------
	// ---------------------------------------------------------------
...
FUNC VOID STARTUP_ORCTEMPEL ()
{
	...
	Wld_InsertNpc				( ORC_Priest_5, "TPL_300" ); 
	//Wld_SendTrigger("WYZWALACZ");
	//Wld_InsertNpc				(NON_5091_MrocznyZeno,"TPL_ZENEK" ); NIE JEST POTZREBNY!! WATEK CZARNEGO MAGA WYWALONY

	
//	Wld_InsertNpc				(UnDeadShadowbeast,	"TPL_394" );
//	Wld_InsertNpc				(UnDeadShadowbeast,	"TPL_378" );
//	Wld_InsertNpc				(UnDeadShadowbeast,	"TPL_381" );
//	Wld_InsertNpc				(UnDeadShadowbeast,	"TPL_383" );
	// ----------------- Sleeper ---------------------------------------
	Wld_InsertNpc				( SLEEPER, "TPL_395" ); // Schläft die ganze Zeit :)
	Wld_SendTrigger("WYZWALACZ");
};
