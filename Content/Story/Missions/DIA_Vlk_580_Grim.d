...
INSTANCE DIA_Grim_GrimOszust (C_INFO)
{
   npc          = Vlk_580_Grim;
   nr           = 1;
   condition    = DIA_Grim_GrimOszust_Condition;
   information  = DIA_Grim_GrimOszust_Info;
   permanent	= FALSE;
   description	= "Och, widzê, ¿e ktoœ zosta³ Cieniem.";
};

FUNC INT DIA_Grim_GrimOszust_Condition()
{
...
FUNC VOID DIA_Grim_GrimOszust_Info()
{
    AI_Output (other, self ,"DIA_Grim_GrimOszust_15_01"); //Och, widzê, ¿e ktoœ zosta³ Cieniem pod moj¹ nieobecnoœæ.
    AI_Output (self, other ,"DIA_Grim_GrimOszust_03_02"); //Tak. I co z tego? Nie pasuje ci coœ?!
    AI_Output (other, self ,"DIA_Grim_GrimOszust_15_03"); //Tak. To, ¿e awansowa³eœ moim kosztem.
    AI_Output (self, other ,"DIA_Grim_GrimOszust_03_04"); //Tylko spróbuj mi coœ zrobiæ. Tak wszystko przygotowa³em, ¿e gdy tylko mnie zaatakujesz, rzuci siê na ciebie ca³y Obóz.
    AI_Output (other, self ,"DIA_Grim_GrimOszust_15_05"); //Nie wiem, co im nagada³eœ, ale jeszcze tego po¿a³ujesz!
    AI_Output (self, other ,"DIA_Grim_GrimOszust_03_06"); //Zobaczymy! 
    B_LogEntry                     (CH1_GrimOc,"Grim ma mnie w garœci. Nie mogê nic mu zrobiæ. Muszê czekaæ na odpowiedni moment. Przypuszczam, ¿e nadejdzie ju¿ wkrótce.");
};
//#####################################################################
//##
//##
...
INSTANCE DIA_Grim_Aroghance (C_INFO)
{
   npc          = Vlk_580_Grim;
   nr           = 1;
   condition    = DIA_Grim_Aroghance_Condition;
   information  = DIA_Grim_Aroghance_Info;
   permanent	= FALSE;
   important	 = true;
   description	= "Kogo ja widzê! Przecie¿ to oszust Grim.";
};

FUNC INT DIA_Grim_Aroghance_Condition()
{
    if (MIS_GrimOc == LOG_RUNNING) && (Kapitel >= 4)
    {
    return TRUE;
    };
};
...
FUNC VOID DIA_Grim_Aroghance_Info()
{
    AI_Output (other, self ,"DIA_Grim_Aroghance_15_01"); //Kogo ja widzê! Przecie¿ to oszust Grim. Widzê, ¿e Gomez nagrodzi³ ciê za obrabianie mu dupy!
    AI_Output (self, other ,"DIA_Grim_Aroghance_03_02"); //Jeszcze ¿yjesz? Naprawdê masz wiêcej szczêœcia ni¿ rozumu!
    AI_Output (other, self ,"DIA_Grim_Aroghance_15_03"); //Za chwilê nie bêdzie ci tak do œmiechu!
    AI_Output (self, other ,"DIA_Grim_Aroghance_03_04"); //Nas jest trzech, a ty tylko jeden.
    AI_Output (other, self ,"DIA_Grim_Aroghance_15_05"); //To wci¹¿ za ma³o.
    AI_Output (self, other ,"DIA_Grim_Aroghance_03_06"); //Zobaczymy! 
	MIS_GrimOc == LOG_SUCCESS;
	Log_SetTopicStatus       (CH1_GrimOc, LOG_SUCCESS);
    B_LogEntry                     (CH1_GrimOc,"Nadszed³ czas na zemstê. Grim awansowa³ na drabinie spo³ecznej Starego Obozu, jednak lepsza zbroja nie czyni z niego lepszego wojownika.");

	Log_CreateTopic(Bossowie, LOG_NOTE);
	B_LogEntry (Bossowie, "Grim nieŸle siê dorobi³ na podlizywaniu siê Gomezowi. Dosta³ nowy pancerz, a w jego d³oni dostrzeg³em magiczny miecz zapewne skradziony z siedziby Magów Ognia. Nadszed³ czas zemsty na tym sukinsynie.");
	AI_StopProcessInfos	(self);
	Npc_SetPermAttitude(self,ATT_HOSTILE);
	Npc_SetPermAttitude(GRD_237_Gardist,ATT_HOSTILE);
	Npc_SetPermAttitude(GRD_238_Gardist,ATT_HOSTILE);
	Npc_SetTarget(self,other);
	AI_StartState(self,ZS_ATTACK,1,"");
};
