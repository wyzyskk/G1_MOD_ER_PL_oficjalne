...
{
	if ( Npc_IsInState(self,ZS_Talk) && (Kapitel < 3) && (player_chose_WaterMages == false) )
	{
		return 1;
	};
};
...
instance  KDW_600_Saturas_WannaMage_Run (C_INFO)
{
	npc			=  KDW_600_Saturas;
	nr			=  3;	
	condition	=  KDW_600_Saturas_WannaMage_Run_Condition;
	information	=  KDW_600_Saturas_WannaMage_Run_Info;
	permanent	=  0;
	description =  "Chcia³bym zostaæ magiem! Cronos poda³ mi sekretne has³o.";
};                       

FUNC int  KDW_600_Saturas_WannaMage_Run_Condition()
{
	if (Npc_KnowsInfo(hero, KDW_604_Cronos_WannaMage2))
	&& (hero.level >= 10)
...
FUNC VOID  KDW_600_Saturas_WannaMage_Run_Info()
{
	AI_Output (other, self,"DIA_Saturas_WannaMage_Run_15_00"); //Chcia³bym zostaæ magiem! Cronos poda³ mi sekretne has³o.
	AI_Output (self, other,"DIA_Saturas_WannaMage_Run_08_01"); //S³ysza³em ju¿ o twoich osi¹gniêciach w Starej Kopalni. To naprawdê godne podziwu. Twój duch jest dojrza³y, a serce wierne.
	AI_Output (self, other,"DIA_Saturas_WannaMage_Run_08_02"); //Myœlê, ¿e jesteœ gotów, aby przyj¹æ œluby Wody.
};
...
{	
	if ((CorAngar_SendToNC==TRUE) && ((Npc_KnowsInfo(hero, Info_Cronos_SLEEPER)) || (Npc_GetTrueGuild (hero) == GIL_KDW)))
	{
		return TRUE;
	};	
};
...
FUNC void  KDW_600_Saturas_OATH_Info()
{
 ...
 var int Saturas_give_weak_armor;
/*------------------------------------------------------------------------
//							KDWAUFNAHME									//
------------------------------------------------------------------------*/
...
FUNC VOID Info_Saturas_XARDASGO_Info()
{
	...
	AI_Output			(self,hero,"Info_Saturas_XARDASGO_15_07"); //Oprócz tego mo¿esz porozmawiaæ z Lee na ten temat. Swego czasu wysy³a³ na moje polecenie Najemników, aby skontaktowaæ siê z Xardasem. Mo¿e on powie ci coœ wiêcej.
	

	B_LogEntry			(CH4_FindXardas,	"W bibliotece Magów Wody znajduj¹ siê zapiski traktuj¹ce o sposobach walki z takimi istotami. Powinienem poœwiêciæ trochê czasu na przestudiowanie tych manuskryptów. Powinienem równie¿ porozmawiaæ z Lee.");
...
INSTANCE DIA_Saturas_iFindStrangeMap (C_INFO)
{
   ...
   permanent	= FALSE;
   description	= "Znalaz³em dziwn¹ mapê.";
};
...
{
    AI_Output (other, self ,"DIA_Saturas_iFindStrangeMap_15_01"); //Znalaz³em dziwn¹ mapê.
    AI_Output (self, other ,"DIA_Saturas_iFindStrangeMap_03_02"); //Co to za mapa? Poka¿ mi j¹.
    B_UseFakeScroll ();
    AI_Output (self, other ,"DIA_Saturas_iFindStrangeMap_03_03"); //Gdzie to znalaz³eœ?!
    AI_Output (other, self ,"DIA_Saturas_iFindStrangeMap_15_04"); //Przy ciele jednego z o¿ywieñców, przy Opuszczonej Kopalni.
    AI_Output (self, other ,"DIA_Saturas_iFindStrangeMap_03_05"); //Wygl¹da mi to bardzo niepokoj¹co. 
    AI_Output (self, other ,"DIA_Saturas_iFindStrangeMap_03_06"); //Ta mapa zosta³a narysowana przez jakiegoœ mrocznego s³ugê. Œwiadcz¹ o tym znaki wypalone na papierze.
    AI_Output (self, other ,"DIA_Saturas_iFindStrangeMap_03_07"); //Czerwony symbol zapewne oznacza miejsce spotkania.
    AI_Output (self, other ,"DIA_Saturas_iFindStrangeMap_03_08"); //Nowe zagro¿enie zawis³o nad Koloni¹ i to ty musisz je powstrzymaæ.
    AI_Output (other, self ,"DIA_Saturas_iFindStrangeMap_15_09"); //Akurat tym to mnie nie zaskoczy³eœ.
    AI_Output (self, other ,"DIA_Saturas_iFindStrangeMap_03_10"); //Udaj siê w wyznaczone miejsce i sprawdŸ co tam na ciebie czeka.
	if (Npc_KnowsInfo(hero,DIA_Xardas_KillBlacMageFix))
	{
	AI_Output (other, self ,"DIA_Saturas_iFindStrangeMap_15_11"); //Zaczekaj! 
	AI_Output (other, self ,"DIA_Saturas_iFindStrangeMap_15_11"); //Coœ sobie przypomnia³em! O tym zagro¿eniu ostrzega³ mnie ju¿ Xardas.
	AI_Output (self, other ,"DIA_Saturas_iFindStrangeMap_03_12"); //Uwa¿asz, ¿e jest w to zamieszany?
	AI_Output (other, self ,"DIA_Saturas_iFindStrangeMap_15_13"); //Nie. Po prostu kaza³ mi siê tym zaj¹æ.
	AI_Output (self, other ,"DIA_Saturas_iFindStrangeMap_03_14"); //IdŸ wiêc!
	};
    MIS_WlakaZczarym = LOG_RUNNING;

    Log_CreateTopic            (CH1_WlakaZczarym, LOG_MISSION);
    Log_SetTopicStatus       (CH1_WlakaZczarym, LOG_RUNNING);
    B_LogEntry                     (CH1_WlakaZczarym,"Okazuje siê, ¿e mapa, któr¹ znalaz³em zwiastuje nowe zagro¿enie ze strony mrocznych si³. Nie pozostaje mi nic innego jak wyjœæ im na przeciw.");
    B_LogEntry                     (CH1_blackMage,"Pokaza³em mapê Saturasowi. Przynios³a mi nowe wyzwania i k³opoty. Widocznie taki ju¿ jest mój los.");
...
INSTANCE DIA_Saturas_bylekmadas (C_INFO)
{
   ...
   permanent	= FALSE;
   description	= "By³em na miejscu spotkania.";
};
...
{
    AI_Output (other, self ,"DIA_Saturas_bylekmadas_15_01"); //By³em na miejscu spotkania.
    AI_Output (self, other ,"DIA_Saturas_bylekmadas_03_02"); //I? Co tam widzia³eœ?
    AI_Output (other, self ,"DIA_Saturas_bylekmadas_15_03"); //Spotka³em Nidasa. To jakiœ wys³annik mrocznych si³.
    AI_Output (other, self ,"DIA_Saturas_bylekmadas_15_04"); //Jest s³ug¹ niejakiego Czarnego Maga.
    AI_Output (self, other ,"DIA_Saturas_bylekmadas_03_05"); //Musimy jak najszybciej dowiedzieæ siê co planuj¹.
    AI_Output (self, other ,"DIA_Saturas_bylekmadas_03_06"); //Uda³o ci siê ustaliæ coœ jeszcze?
    B_LogEntry                     (CH1_WlakaZczarym,"Zanios³em informacjê do Saturasa. Musimy jak najszybciej dowiedzieæ siê co planuje Czarny Mag i go powstrzymaæ.");
	
    B_GiveXP (500);

    Info_ClearChoices		(DIA_Saturas_bylekmadas);
    Info_AddChoice		(DIA_Saturas_bylekmadas, "Poszukiwacze podlegaj¹ Czarnemu Magowi.", DIA_Saturas_bylekmadas_Poszukiwacze);
    Info_AddChoice		(DIA_Saturas_bylekmadas, "Czarny Mag to trudny przeciwnik.", DIA_Saturas_bylekmadas_CzarnyChuj);
    Info_AddChoice		(DIA_Saturas_bylekmadas, "Mam tu pewn¹ listê.", DIA_Saturas_bylekmadas_List);
    Info_AddChoice		(DIA_Saturas_bylekmadas, "Tyle uda³o mi siê ustaliæ.", DIA_Saturas_bylekmadas_ENDE);
};

FUNC VOID DIA_Saturas_bylekmadas_Poszukiwacze()
{
    AI_Output (other, self ,"DIA_Saturas_bylekmadas_Poszukiwacze_15_01"); //Poszukiwacze podlegaj¹ Czarnemu Magowi.
    AI_Output (other, self ,"DIA_Saturas_bylekmadas_Poszukiwacze_15_02"); //To przez niego zjawili siê w Kolonii. 
    AI_Output (self, other ,"DIA_Saturas_bylekmadas_Poszukiwacze_03_03"); //Wiesz mo¿e czego szukaj¹?
    AI_Output (other, self ,"DIA_Saturas_bylekmadas_Poszukiwacze_15_04"); //Dowiedzia³em siê, ¿e ich celem jest zbieranie dusz niewiernych.
    AI_Output (other, self ,"DIA_Saturas_bylekmadas_Poszukiwacze_15_05"); //Nie wiem jednak po co im te dusze. 
    AI_Output (self, other ,"DIA_Saturas_bylekmadas_Poszukiwacze_03_06"); //Musimy to jak najszybciej ustaliæ.
...
FUNC VOID DIA_Saturas_bylekmadas_CzarnyChuj()
{
    ...
    AI_Output (other, self ,"DIA_Saturas_bylekmadas_CzarnyChuj_15_02"); //Walka z nim nie bêdzie ³atwa.
    AI_Output (self, other ,"DIA_Saturas_bylekmadas_CzarnyChuj_03_03"); //Beliar zsy³a swe s³ugi tylko, gdy ma to jakiœ cel.
    AI_Output (self, other ,"DIA_Saturas_bylekmadas_CzarnyChuj_03_04"); //Przypuszczam, ¿e Czarny Mag jest wys³annikiem czegoœ potê¿niejszego.
    AI_Output (other, self ,"DIA_Saturas_bylekmadas_CzarnyChuj_15_05"); //Co masz na myœli?
    AI_Output (self, other ,"DIA_Saturas_bylekmadas_CzarnyChuj_03_06"); //Ktoœ ma nad nim w³adzê. Mo¿liwe, ¿e przygotowuj¹ siê na przyjœcie tego kogoœ.
    AI_Output (other, self ,"DIA_Saturas_bylekmadas_CzarnyChuj_15_07"); //To bardzo prawdopodobne.
    AI_Output (self, other ,"DIA_Saturas_bylekmadas_CzarnyChuj_03_08"); //Musimy jak najszybciej ustaliæ gdzie znajduje siê Czarny Mag.
...
{
    AI_Output (other, self ,"DIA_Saturas_bylekmadas_List_15_01"); //Mam tu pewn¹ listê.
    AI_Output (self, other ,"DIA_Saturas_bylekmadas_List_03_02"); //Poka¿ mi j¹!
    B_UseFakeScroll ();
    AI_Output (self, other ,"DIA_Saturas_bylekmadas_List_03_03"); //To lista ofiar. Lista celów dla Poszukiwaczy.
    AI_Output (self, other ,"DIA_Saturas_bylekmadas_List_03_04"); //Musisz jak najszybciej porozmawiaæ z tymi osobami.
    AI_Output (self, other ,"DIA_Saturas_bylekmadas_List_03_05"); //Mo¿e dadz¹ ci jak¹œ wskazówkê.
    AI_Output (other, self ,"DIA_Saturas_bylekmadas_List_15_06"); //Natychmiast wyruszê w drogê.
...
{
    AI_Output (other, self ,"DIA_Saturas_bylekmadas_ENDE_15_01"); //Tyle uda³o mi siê ustaliæ.
    AI_Output (self, other ,"DIA_Saturas_bylekmadas_ENDE_03_02"); //Musisz jak najszybciej odnaleŸæ Czarnego Maga i powstrzymaæ jego dzia³ania.
    B_LogEntry                     (CH1_WlakaZczarym,"Pora odnaleŸæ Czarnego Maga i stawiæ mu czo³a. Uprzednio jednak muszê znaleŸæ jego kryjówkê.");
    Info_ClearChoices		(DIA_Saturas_bylekmadas);
    AI_StopProcessInfos	(self);
};
...
INSTANCE DIA_Saturas_DajMiLek (C_INFO)
{
   ...
   permanent	= FALSE;
   description	= "Pilnie potrzebujê leku na zatrucia!";
};
...
{
    AI_Output (other, self ,"DIA_Saturas_DajMiLek_15_01"); //Pilnie potrzebujê leku na zatrucia!
    AI_Output (self, other ,"DIA_Saturas_DajMiLek_03_02"); //Co takiego siê sta³o?
    AI_Output (other, self ,"DIA_Saturas_DajMiLek_15_03"); //Jeden z ³owców Poszukiwaczy zosta³ zraniony zatrut¹ strza³¹.
    AI_Output (self, other ,"DIA_Saturas_DajMiLek_03_04"); //O, Adanosie! 
    AI_Output (self, other ,"DIA_Saturas_DajMiLek_03_05"); //Pogadaj z Riordianem. Mo¿e coœ ci uwarzy.

    B_LogEntry                     (CH1_lekiDlaAva,"Riordian mo¿e mi uwarzyæ miksturê. Muszê z nim pogadaæ.");
...
INSTANCE DIA_Saturas_OpuszczonaOdbitaNc (C_INFO)
{
   ...
   permanent	= FALSE;
   description	= "Odbi³em Opuszczon¹ Kopalniê.";
};
...
{
    AI_Output (other, self ,"DIA_Saturas_OpuszczonaOdbitaNc_15_01"); //Odbi³em Opuszczon¹ Kopalniê.
    AI_Output (other, self ,"DIA_Saturas_OpuszczonaOdbitaNc_15_02"); //Cord kaza³ ci zaproponowaæ, abyœmy wznowili w niej wydobycie.
    AI_Output (self, other ,"DIA_Saturas_OpuszczonaOdbitaNc_03_03"); //To znacznie by nam pomog³o.
    AI_Output (self, other ,"DIA_Saturas_OpuszczonaOdbitaNc_03_04"); //Potrzebujemy ludzi do kopalni.
    AI_Output (other, self ,"DIA_Saturas_OpuszczonaOdbitaNc_15_05"); //Wyœlijmy kilku Szkodników.
    AI_Output (self, other ,"DIA_Saturas_OpuszczonaOdbitaNc_03_06"); //Wydaje mi siê, ¿e to nie wystarczy. Potrzebujemy wiêcej r¹k do pracy.
    AI_Output (self, other ,"DIA_Saturas_OpuszczonaOdbitaNc_03_07"); //Mo¿e uda siê namówiæ Bandytów do pomocy. 
...
{
    AI_Output (other, self ,"DIA_Saturas_PertractacjieDD_15_01"); //Dogada³em siê z Quentinem.
    AI_Output (other, self ,"DIA_Saturas_PertractacjieDD_15_02"); //Nie by³ zbyt zadowolony, ale siê zgodzi³.
    AI_Output (self, other ,"DIA_Saturas_PertractacjieDD_03_03"); //Œwietnie. 
...
INSTANCE DIA_Saturas_GRATULACJE_Z_KOPALNIA (C_INFO)
{
   npc          = KDW_600_Saturas;
   nr           = 1;
   condition    = DIA_Saturas_GRATULACJE_Z_KOPALNIA_Condition;
   information  = DIA_Saturas_GRATULACJE_Z_KOPALNIA_Info;
   permanent	= FALSE;
   Important    = TRUE;
};

FUNC INT DIA_Saturas_GRATULACJE_Z_KOPALNIA_Condition()
{
    if (Npc_KnowsInfo (hero, DIA_lee_PertractacjieDD))
    {
    return TRUE;
    };
};
...
INSTANCE DIA_Saturas_KillCzarnyDebil (C_INFO)
{
   ...
   permanent	= FALSE;
   description	= "Walczy³em z Czarnym Magiem!";
};
...
{
    AI_Output (other, self ,"DIA_Saturas_KillCzarnyDebil_15_01"); //Walczy³em z Czarnym Magiem!
    AI_Output (other, self ,"DIA_Saturas_KillCzarnyDebil_15_02"); //Pokona³em go w walce!
    AI_Output (other, self ,"DIA_Saturas_KillCzarnyDebil_15_03"); //Ten przebieg³y czarodziej by³ wys³annikiem kogoœ innego.
    AI_Output (other, self ,"DIA_Saturas_KillCzarnyDebil_15_04"); //Podejrzewam, ¿e to mo¿e mieæ zwi¹zek ze Œni¹cym.
    AI_Output (other, self ,"DIA_Saturas_KillCzarnyDebil_15_05"); //Mag chcia³ przygotowaæ œwiat na jego przyjœcie.
    AI_Output (other, self ,"DIA_Saturas_KillCzarnyDebil_15_06"); //Wykorzystywa³ do tego dusze ludzi.
    AI_Output (self, other ,"DIA_Saturas_KillCzarnyDebil_03_07"); //To okropne! Musimy czym prêdzej siê st¹d wydostaæ.
    AI_Output (other, self ,"DIA_Saturas_KillCzarnyDebil_15_08"); //Na razie nic nam nie grozi. Zada³em potê¿ny cios Beliarowi.
    AI_Output (self, other ,"DIA_Saturas_KillCzarnyDebil_03_09"); //Masz racjê, ale to, ¿e Czarny Mag nie ¿yje nie oznacza, ¿e Kolonia jest ju¿ bezpieczna!
    B_LogEntry                     (CH1_WlakaZczarym,"Czarny Mag nie ¿yje. Poinformowa³em o tym Saturasa. Nie zmienia to jednak faktu, ¿e nadal nie jesteœmy ca³kiem bezpieczni.");
